...
import { uploadImagesInBatches } from '../lib/utils';
...
export default function GalleryViewerPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Usar uploadImagesInBatches para optimizar subida
      const uploadedUrls = await uploadImagesInBatches(
        files,
        3, // Subir 3 imágenes a la vez
        (index, progress, status) => {
          setUploadProgress(prev => {
            ...
            const overallPercent = totalProgress / updated.length;
