...
import { uploadToImgBB, uploadImagesInBatches } from '../lib/utils';
...
// Helper functions to handle pages that can be strings or objects
type PageData = string | { originalUrl: string; thumbnailUrl?: string };

const getPageUrl = (page: PageData): string => {
...
const getPageThumbnailUrl = (page: PageData): string => {
  if (typeof page === 'string') {
    return page;
  }
  return page.thumbnailUrl || page.originalUrl;
};

// Componente memoizado para vista previa de imágenes nuevas (optimizado para memoria)
interface NewImagesPreviewProps {
  versionId: string;
  files: File[];
  existingPagesCount: number;
  coverImagePreview: string | null;
  coverImageFile: File | null;
  selectedCoverPageIndex: { versionId: string; pageIndex: number } | null;
  dragOverNewImageIndex: number | null;
  uploadProgress: UploadProgress[];
  onDragStart: (e: React.DragEvent, index: number) => void;
  onDragOver: (e: React.DragEvent, index: number) => void;
  onDragEnd: () => void;
  onDrop: (e: React.DragEvent, dropIndex: number) => void;
  onClick: (index: number, previewUrl: string) => void;
  onRemove: (index: number) => void;
  objectUrlsRef: React.MutableRefObject<Set<string>>;
}

const NewImagesPreview = React.memo(({
  versionId,
  files,
  existingPagesCount,
  coverImagePreview,
  ...
  onRemove,
  objectUrlsRef
}: NewImagesPreviewProps) => {
  // Memoizar URLs de objeto para evitar recrearlas en cada render
  ...
  }, [previewUrls, objectUrlsRef]);

  return (
    <div className="mb-6">
      <h5 className="text-sm font-medium text-gray-400 mb-3 flex items-center gap-2">
        <div className="w-1.5 h-1.5 bg-primary rounded-full"></div>
        Nuevas Páginas ({files.length}) - Arrastra para reordenar
      </h5>
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
        {files.map((file, index) => {
          const previewUrl = previewUrls[index];
          const isSelectedAsCover = coverImagePreview === previewUrl || 
            (selectedCoverPageIndex?.versionId === versionId && 
             selectedCoverPageIndex?.pageIndex === -1 && 
             coverImageFile === file);
...
NewImagesPreview.displayName = 'NewImagesPreview';
...
export default function MangaForm({ 
  ...
  const objectUrlsRef = useRef<Set<string>>(new Set()); // Para limpiar URLs de objeto

  const mangaCategories = useMemo(() => categories.filter(c => c.type === 'manga'), [categories]);

  // Limpiar URLs de objeto cuando el componente se desmonte o cambien las imágenes
  useEffect(() => {
    return () => {
  ...
  // Limpiar URLs de objeto de imágenes eliminadas
  ...
  const handleCoverImageSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (coverImagePreview && coverImagePreview.startsWith('blob:')) {
        cleanupObjectUrl(coverImagePreview);
      }
  ...
  }, [coverImagePreview, cleanupObjectUrl]);
  ...
  const handleImageSelect = useCallback((versionId: string, e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    
    // Validar tamaño de archivos antes de agregarlos
    const validFiles = files.filter(file => {
      if (file.size > API_CONFIG.IMGBB.MAX_FILE_SIZE) {
        console.warn(`Archivo ${file.name} es demasiado grande (${(file.size / 1024 / 1024).toFixed(2)}MB). Se comprimirá antes de subir.`);
        return true; // Permitir pero se comprimirá después
      }
      return true;
    });
    ...
    setSelectedImages(prev => ({
      ...prev,
      [versionId]: [...(prev[versionId] || []), ...validFiles]
    }));
  ...
  const handleRemoveImage = useCallback((versionId: string, index: number, isExisting: boolean = false) => {
    if (isExisting) {
      ...
      // Remove from new selected images and cleanup object URLs
      setSelectedImages(prev => {
        const versionImages = prev[versionId] || [];
        const fileToRemove = versionImages[index];
        
        // No podemos limpiar la URL aquí porque se crea en el render
        // Se limpiará cuando se desmonte el componente o cuando se cambie la lista
        
        return {
          ...prev,
          [versionId]: versionImages.filter((_, i) => i !== index)
        };
      });
      setUploadProgress(prev => ({
        ...prev,
  ...
  const handlePageClick = useCallback((versionId: string, pageIndex: number, pageUrl: string, isNewUpload: boolean = false) => {
    // Limpiar preview anterior si existe
    if (coverImagePreview && coverImagePreview.startsWith('blob:')) {
      cleanupObjectUrl(coverImagePreview);
    }
    ...
    if (isNewUpload) {
      // Handle selection from new uploads - pageUrl ya es la URL de objeto
      setCoverImagePreview(pageUrl);
      setSelectedImages(current => {
        const newImages = current[versionId];
  ...
  const moveNewImageInVersion = useCallback((versionId: string, fromIndex: number, toIndex: number) => {
    setSelectedImages(prev => {
      const versionImages = [...(prev[versionId] || [])];
  ...
  const handleNewImageDragStart = useCallback((e: React.DragEvent, index: number) => {
    setDraggedNewImageIndex(index);
    e.dataTransfer.effectAllowed = 'move';
  }, []);

  const handleNewImageDragOver = useCallback((e: React.DragEvent, index: number) => {
    e.preventDefault();
    setDragOverNewImageIndex(index);
  }, []);

  const handleNewImageDragEnd = useCallback(() => {
    setDraggedNewImageIndex(null);
    setDragOverNewImageIndex(null);
  }, []);

  const handleNewImageDrop = useCallback((e: React.DragEvent, versionId: string, dropIndex: number) => {
    e.preventDefault();
    const currentIndex = draggedNewImageIndex;
    if (currentIndex !== null && currentIndex !== dropIndex) {
      moveNewImageInVersion(versionId, currentIndex, dropIndex);
    }
    setDraggedNewImageIndex(null);
    setDragOverNewImageIndex(null);
  }, [draggedNewImageIndex, moveNewImageInVersion]);
  ...
  const updateProgress = useCallback((versionId: string, index: number, progress: number) => {
    setUploadProgress(prev => {
      const updated = {
        ...prev,
      ...
      const allProgress = Object.values(updated).flat();
    ...
    try {
      ...
      if (coverImageFile) {
        ...
        if (selectedVersion && selectedVersion.pages[selectedCoverPageIndex.pageIndex]) {
          const page = selectedVersion.pages[selectedCoverPageIndex.pageIndex];
          coverImageUrl = getPageUrl(page as PageData);
        }
      } else if (!coverImageUrl && versions.length > 0 && versions[0].pages.length > 0) {
        // Auto-select first page as cover if no cover is set
        const firstPage = versions[0].pages[0];
        coverImageUrl = getPageUrl(firstPage as PageData);
      }

      // Upload images for each version using batch upload
      ...
      for (const version of versions) {
        ...
        if (versionImages.length > 0) {
          // Upload new images for this version using batch upload (3 at a time)
          const uploadedUrls = await uploadImagesInBatches(
            versionImages,
            3, // Subir 3 imágenes a la vez para no sobrecargar memoria
            (index, progress, status) => {
              ...
              setUploadProgress(prev => ({
                ...
                [version.id]: (prev[version.id] || []).map((item, i) => 
                  i === index 
                    ? { ...item, progress, status: status === 'completed' ? 'completed' : status === 'error' ? 'error' : 'uploading' } 
                    : item
                )
              }));
            }
                ...
                <div key={version.id} className="bg-surface/50 rounded-xl p-6 border border-gray-600/30">
                    ...
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-2">
                        {version.pages.map((page, index) => {
                          const pageThumbnailUrl = getPageThumbnailUrl(page as PageData);
                          const pageUrl = getPageUrl(page as PageData);
                          ...
                          return (
                            ...
                            >
                              <img
                                ...
                                alt={`Página ${index + 1}`}
                                className={`w-full h-full object-cover rounded-lg ring-1 transition-all cursor-pointer ${
                                  selectedCoverPageIndex?.versionId === version.id && selectedCoverPageIndex?.pageIndex === index
                                    ? 'ring-2 ring-primary shadow-lg shadow-primary/25'
                  ...
                  {selectedImages[version.id] && selectedImages[version.id].length > 0 && (
                    <NewImagesPreview
                      ...
                      files={selectedImages[version.id]}
                      existingPagesCount={version.pages?.length || 0}
                      coverImagePreview={coverImagePreview}
                      coverImageFile={coverImageFile}
                      selectedCoverPageIndex={selectedCoverPageIndex}
                      dragOverNewImageIndex={dragOverNewImageIndex}
                      uploadProgress={uploadProgress[version.id] || []}
                      onDragStart={handleNewImageDragStart}
                      onDragOver={handleNewImageDragOver}
                      onDragEnd={handleNewImageDragEnd}
                      onDrop={(e, dropIndex) => handleNewImageDrop(e, version.id, dropIndex)}
                      onClick={(index, previewUrl) => handlePageClick(version.id, index, previewUrl, true)}
                      onRemove={(index) => handleRemoveImage(version.id, index, false)}
